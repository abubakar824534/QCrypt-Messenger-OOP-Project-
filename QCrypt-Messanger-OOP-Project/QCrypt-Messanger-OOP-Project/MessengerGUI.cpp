...
MessengerGUI::MessengerGUI() :activeContact(0), selectedCipher(0), inputLen(0), scrollOffset(0), cipherDropdownOpen(false), backspaceTimer(0), eveMode(false)
{
...
void MessengerGUI::drawText(const char* text, float x, float y, float fontSize, Color color)
{
    Vector2 pos;
    pos.x = x;
    pos.y = y;
...
float MessengerGUI::measureText(const char* text, float fontSize)
{
    if (mainFont.texture.id == 0)
    {
...
int MessengerGUI::getChatHeight(int contactIdx)
{
...
void MessengerGUI::loadChat(int idx)
{
...
void MessengerGUI::saveMessage(int idx, const Message& msg)
{
    MyString chatFile = contacts[idx].getChatFile();
...
void MessengerGUI::drawChatHeader()
{
...
void MessengerGUI::drawChatMessages()
{
...
void MessengerGUI::drawInputBar()
{
    if (activeContact < 0)
    {
        return;
...
void MessengerGUI::drawCipherDropdown()
{
...
void MessengerGUI::sendCurrentMessage()
{
    if (inputLen == 0)
    {
        return;
